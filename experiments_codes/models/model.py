...
class DecoderWrapper:
    ...
    def _get_mask_token_positions(self, inputs: Dict[str, torch.Tensor]):
        ...
        for key in sorted(masked_index.keys()):
            masked_rows.append(key)
            masked_cols.append(max(masked_index[key]))
        return masked_rows, masked_cols
        ...
        with torch.no_grad():
            for i in tqdm(range(0, batch_cnt)):
                ...
                if len(selected_layers) == 0 or -1 in selected_layers:
                    ...
                    ffn_intervention = [ self.model.model.model.layers[i].output[:,-1:,:] for i in range(len(self.model.model.model.layers))]
                    mono_hidden_states = [ self.model.model.model.layers[i].ffn_states for i in range(len(self.model.model.model.layers))]
                    ...
                    cs_hidden_states = [ self.model.model.model.layers[i].ffn_states for i in range(len(self.model.model.model.layers))]
